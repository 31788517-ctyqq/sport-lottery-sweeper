...
async def test_submit_issue_precise():
    """使用更精确的选择器测试提交问题"""
    async with async_playwright() as p:
        ...
        try:
            ...
            print("选择分类...")
            # 先通过标签定位到分类选择器的容器，然后点击选择器
            await page.locator('label:has-text("分类")').locator('xpath=following-sibling::div//div[@class="el-select__wrapper"]').click()
            ...
            if success_msg > 0:
                ...
                # 截图用于调试
                await page.screenshot(path='precise_test_debug.png')
                
        except Exception as e:
            print(f"测试失败: {str(e)}")
            await page.screenshot(path='precise_test_error.png')
            raise e
        finally:
            await browser.close()
...
if __name__ == "__main__":
    asyncio.run(test_submit_issue_precise())