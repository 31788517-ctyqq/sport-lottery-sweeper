...
async def test_create_and_edit_using_focus():
    """使用focus和键盘事件的测试"""
    async with async_playwright() as p:
        ...
        try:
            ...
            print("6. 使用focus和键盘事件选择分类...")
            # 使用JavaScript直接操作select元素
            await page.evaluate("""
                // 直接通过prop="category"找到分类选择器
                const categorySelect = document.querySelector('label:has-text("分类") + .el-form-item__content .el-select input');
                if (categorySelect) {
                    categorySelect.focus();
                    categorySelect.click();  // 模拟点击
                }
            """)
            ...
            try:
                ...
                await page.screenshot(path='focus_keyboard_test_creation_failed.png')
                return  # 如果创建失败则退出测试
            ...
            try:
                ...
                await page.screenshot(path='focus_keyboard_test_update_failed.png')
                
            print("✅ Focus和键盘事件测试流程结束")
                
        except Exception as e:
            print(f"测试过程中出现错误: {str(e)}")
            await page.screenshot(path='focus_keyboard_test_error.png')
            raise e
        finally:
            await browser.close()
...
if __name__ == "__main__":
    asyncio.run(test_create_and_edit_using_focus())