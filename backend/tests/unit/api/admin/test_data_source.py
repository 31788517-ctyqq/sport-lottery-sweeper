...
from backend.schemas.sp_management import DataSourceFilterParams, DataSourceCreate, DataSourceUpdate
...
class TestDataSourceAPI:
    ...
    def setup(self):
        ...
        self.mock_service.get_data_source.return_value = create_mock_source(self.sample_source)
        self.mock_service.create_data_source.return_value = create_mock_source(self.sample_source)
        self.mock_service.update_data_source.return_value = create_mock_source(self.sample_source)
        ...
        self.service_patcher = patch('backend.api.v1.admin.data_source.SPManagementService', return_value=self.mock_service)
    ...
    def test_test_data_source_connection_success(self):
        ...
        # 验证服务被调用
        self.mock_service.test_data_source.assert_called_once_with(1)
    ...
    def test_batch_delete_data_sources_success(self):
        ...
        # 验证服务被调用
        self.mock_service.batch_delete_data_sources.assert_called_once_with([1, 2, 3])
    ...
    def test_batch_health_check_success(self):
        ...
        self.mock_service.test_data_source.side_effect = [
            {"success": True, "response_time": 100, "status_code": 200, "message": "成功"},
            {"success": False, "response_time": 500, "status_code": 500, "message": "失败"}
        ]
    ...
    def test_batch_update_status_success(self):
        """测试批量更新状态成功"""
        # 这个端点目前是临时实现，直接返回成功
        response = self.client.post("/api/v1/admin/sources/batch-update-status")
