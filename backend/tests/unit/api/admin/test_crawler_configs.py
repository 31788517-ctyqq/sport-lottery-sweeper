...
from backend.api.v1.admin.crawler_configs import router
...
class TestCrawlerConfigsAPI:
    ...
    def test_get_configs_content_structure(self, test_client):
        ...
        headers = global_config["content"]["headers"]
        assert headers["User-Agent"] == "default-agent"
    ...
    def test_config_data_types(self, test_client):
        ...
        for config in data:
            ...
            # 验证时间格式（ISO格式）
            try:
                created_at = config["created_at"].replace('Z', '+00:00')
                updated_at = config["updated_at"].replace('Z', '+00:00')
    ...
    def test_config_endpoint_resilience(self, test_client):
        ...
        response = test_client.put(
            f"/api/v1/admin/crawler/config/{invalid_config_id}"
        )
