...
class TestDataCenterAPI:
    ...
    def test_get_data_center_overview_success(self, test_client, mock_db):
        ...
        mock_db.query.return_value.filter.return_value.count.return_value = 50  # active_sources
    ...
    def test_get_data_trend_success(self, test_client, mock_db):
        """测试获取数据趋势成功"""
        response = test_client.get("/api/v1/admin/data-center/data-trend", params={"days": 30})
    ...
    def test_get_data_trend_with_custom_days(self, test_client):
        """测试使用自定义天数获取数据趋势"""
        response = test_client.get("/api/v1/admin/data-center/data-trend", params={"days": 7})
    ...
    def test_get_data_trend_date_range(self, test_client):
        """测试数据趋势的日期范围"""
        response = test_client.get("/api/v1/admin/data-center/data-trend", params={"days": 10})
        ...
        # 验证开始日期和结束日期
        start_date = datetime.fromisoformat(summary["startDate"].replace('Z', '+00:00')).date()
        end_date = datetime.fromisoformat(summary["endDate"].replace('Z', '+00:00')).date()
    ...
    def test_get_data_trend_data_consistency(self, test_client):
        """测试数据趋势的数据一致性"""
        response = test_client.get("/api/v1/admin/data-center/data-trend", params={"days": 5})
    ...
    def test_get_data_sources_with_filters(self, test_client, mock_db, mock_query_result):
        """测试带筛选条件的数据源列表获取"""
        mock_db.query.return_value = mock_query_result
    ...
    def test_get_detail_data_success(self, test_client, mock_db):
        ...
        detail_data = {
            "matches": [
                {"id": 1, "home_team": "Team A", "away_team": "Team B", "status": "upcoming"}
            ],
            "odds": [
    ...
    def test_export_data_success(self, test_client, mock_db):
        """测试导出数据成功"""
        response = test_client.get("/api/v1/admin/data-center/export-data", params={"format": "json"})
    ...
    def test_export_data_with_csv_format(self, test_client, mock_db):
        """测试以CSV格式导出数据"""
        response = test_client.get("/api/v1/admin/data-center/export-data", params={"format": "csv"})
    ...
    def test_export_data_with_invalid_format(self, test_client):
        """测试使用无效格式导出数据"""
        response = test_client.get("/api/v1/admin/data-center/export-data", params={"format": "invalid"})
    ...
    def test_data_trend_date_format(self, test_client):
        """测试数据趋势的日期格式"""
        response = test_client.get("/api/v1/admin/data-center/data-trend", params={"days": 3})
