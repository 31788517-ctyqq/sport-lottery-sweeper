...
class TestMatchService:
    ...
    def test_create_match_success(self, match_service, mock_db, sample_team, sample_league):
        """测试成功创建比赛"""
        # 模拟查询结果
    ...
    def test_create_match_past_date(self, match_service, mock_db, sample_team, sample_league):
        """测试创建比赛 - 过去日期"""
        # 模拟查询结果
        mock_db.query.return_value.filter.return_value.first.side_effect = [
        ...
        with pytest.raises(ValidationException, match='比赛日期不能是过去的时间'):
            match_service.create_match(match_data)
    
    def test_get_match_by_id_success(self, match_service, mock_db, sample_match):
        """测试根据ID获取比赛 - 成功"""
        # 模拟查询结果
        mock_db.query.return_value.filter.return_value.first.return_value = sample_match
    ...
    def test_update_match_not_found(self, match_service, mock_db):
        ...
        with pytest.raises(NotFoundException, match='比赛'):
            match_service.update_match(999, {'venue': 'Test'})
    
    def test_update_match_status_success(self, match_service, mock_db, sample_match):
        """测试更新比赛状态 - 成功"""
        # 模拟查询结果
        mock_db.query.return_value.filter.return_value.first.return_value = sample_match
        ...
        result = match_service.update_match_status(1, MatchStatusEnum.LIVE)
    ...
    def test_get_upcoming_matches(self, match_service, mock_db):
        ...
        mock_db.query.return_value.filter.return_value.order_by.return_value.all.return_value = mock_matches
