...
class FiveHundredProxyScraper(BaseScraper):
    """
    500彩票网竞彩足球爬虫 (代理版)
    ...
    def __init__(self, engine: Optional[ScraperEngine] = None):
        ...
        # 不创建自己的session，而是使用代理IP
        self.session = None
    
    def get_source_name(self) -> str:
        return "500彩票网竞彩足球(代理版)"
    ...
    async def get_matches(self, days: int = 3) -> List[Dict[str, Any]]:
        ...
        """
        try:
            # 尝试直接访问，如果有反爬虫问题再考虑其他方法
            return await self._fetch_without_proxy(days)
            
        except Exception as e:
            logger.error(f"Error scraping 500彩票网: {str(e)}")
            return await self._fetch_without_proxy(days)
    
    async def _fetch_without_proxy(self, days: int = 3) -> List[Dict[str, Any]]:
        """
        不使用代理直接获取数据
        """
        logger.info("尝试直接访问500彩票网")
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            # 先访问主页建立会话
            await self._visit_homepage_with_session(session, headers)
            ...
            try:
                async with session.get(self.BASE_URL, headers=headers) as response:
                    if response.status != 200:
                        logger.error(f"直接访问失败，状态码: {response.status}")
                        return []
                ...
                logger.info(f"直接访问成功，获取到 {len(matches)} 场比赛")
                return matches
                
            except Exception as e:
                logger.error(f"直接访问也失败了: {str(e)}")
                return []
    ...
    async def _visit_homepage_with_session(self, session, headers: dict):
        """
        使用会话访问主页建立会话
        """
        try:
...
# 为兼容性提供一个函数接口
async def scrape_five_hundred_jczq_with_proxy(days: int = 3) -> List[Dict[str, Any]]:
    """
    使用代理爬取500彩票网竞彩足球数据的便捷函数
    ...
    Returns:
        List[Dict]: 比赛数据列表
    """
    scraper = FiveHundredProxyScraper()
    try:
