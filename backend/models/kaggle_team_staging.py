...
class KaggleTeamStaging(Base):
    __tablename__ = "kaggle_team_staging"

    id = Column(Integer, primary_key=True, index=True)
    dataset_slug = Column(String(255), nullable=False)
    version = Column(String(64), nullable=False)
    external_id = Column(String(255), nullable=False)
    team_name_raw = Column(String(255), nullable=True)
    ...
    __table_args__ = (
        UniqueConstraint("dataset_slug", "version", "external_id", name="uq_kaggle_team_staging_identity"),
        Index("idx_kaggle_team_staging_hash", "normalized_hash"),
    )
