...
async def test_keyboard_selection():
    """使用键盘事件选择下拉选项"""
    async with async_playwright() as p:
        ...
        try:
            ...
            print("6. 使用键盘事件选择分类...")
            # 找到分类下拉框的输入区域并聚焦
            category_input = page.locator('label:has-text("分类")').nth(1).locator('xpath=following::div[1]//input[@class="el-select__input"]')
            
            # 如果找不到输入框，则尝试直接点击select的wrapper
            if await category_input.count() == 0:
                ...
                await page.evaluate("""
                    ...
                    if (categoryLabels.length >= 2) {
                        const targetLabel = categoryLabels[1]; // 第二个分类标签
            ...
            # 使用键盘选择选项
            print("7. 使用键盘选择'赛事数据'...")
            # 使用Page.down()方法模拟键盘事件
            await page.keyboard.press('ArrowDown')  # 向下箭头选择第一个选项
            ...
            try:
                ...
                await page.screenshot(path='keyboard_selection_test_creation_failed.png')
                return  # 如果创建失败则退出测试
            ...
            await page.keyboard.press('ArrowDown')  # 向下箭头选择第一个选项
            await page.wait_for_timeout(200)
            await page.keyboard.press('ArrowDown')  # 再向下选择第二个选项
            await page.wait_for_timeout(200)
            await page.keyboard.press('Enter')  # 确认选择
            ...
            try:
                ...
                await page.screenshot(path='keyboard_selection_test_update_failed.png')
                
            print("✅ 键盘选择测试流程结束")
                
        except Exception as e:
            print(f"测试过程中出现错误: {str(e)}")
            await page.screenshot(path='keyboard_selection_test_error.png')
            raise e
        finally:
            await browser.close()
...
if __name__ == "__main__":
    asyncio.run(test_keyboard_selection())