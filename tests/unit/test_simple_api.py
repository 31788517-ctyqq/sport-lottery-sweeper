...
try:
    ...
    if crawler_routes:
        print(f"✅ 找到 {len(crawler_routes)} 个 crawler 路由")
    else:
        print("❌ 未找到 crawler 路由")
        
except Exception as e:
    print(f"❌ API v1 router 导入失败: {e}")
    import traceback
    traceback.print_exc()