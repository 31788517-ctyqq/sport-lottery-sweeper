...
async def test_complete_crawler_workflow():
    ...
    try:
        ...
        print(f"⚠️  获取比赛数据时出现问题: {str(e)}，使用模拟数据继续测试")
        matches = [
            {
                "match_id": "test_001",
                "home_team": "Home Team",
    ...
    print(f"✅ 从比赛数据中识别出 {len(leagues)} 个不同联赛")
...
async def test_complete_admin_workflow():
    ...
    async with AsyncSessionLocal() as db_session:
        ...
        try:
            ...
            print(f"✅ 成功创建管理员用户: {created_admin.username}")
            ...
            print(f"❌ 管理员工作流程测试失败: {str(e)}")
            raise
    
    print("🎉 完整的管理员工作流程测试通过")


@pytest.mark.asyncio
async def test_complete_crawler_config_workflow():
    """测试完整的爬虫配置工作流程"""
    print("🧪 开始测试完整的爬虫配置工作流程...")
    ...
    async with AsyncSessionLocal() as db_session:
        ...
        try:
            ...
            print(f"✅ 成功创建爬虫配置: {created_config.name}")
            ...
            print(f"❌ 爬虫配置工作流程测试失败: {str(e)}")
            raise
    
    print("🎉 完整的爬虫配置工作流程测试通过")
...
async def test_complete_business_scenario():
    ...
    async with AsyncSessionLocal() as db_session:
        ...
        print(f"✅ 创建业务场景管理员: {admin.username}")
        ...
        print(f"✅ 管理员 {admin.username} 创建了爬虫配置: {config.name}")
        ...
        print(f"✅ 验证了管理员与其创建的配置之间的关联")
        
        # 4. 模拟运行爬虫任务（获取数据）
        try:
            matches = await advanced_crawler.crawl_sporttery_matches(1)
            print(f"✅ 爬虫配置成功获取到 {len(matches)} 条比赛数据")
        except Exception as e:
            print(f"⚠️  获取比赛数据时出现问题: {str(e)}，使用模拟数据")
