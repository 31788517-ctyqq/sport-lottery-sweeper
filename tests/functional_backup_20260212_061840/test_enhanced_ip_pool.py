...
def test_basic_functionality():
    ...
    print(f"[OK] 添加了 {len(test_proxies)} 个测试代理")
    
    # 检查IP池状态
    status = pool.get_status()
    print(f"[ANALYTICS] IP池状态: 总代理数={status['pool_status']['total_proxies']}")
    
    # 获取当前状态
    current_status = pool.proxy_pool_monitor.get_current_status()
    print(f"[TREND] 当前状态: 高质IP={current_status['high_quality_proxies']}")
    
    # 模拟代理使用
    print("[COUNTERCLOCKWISE_ARROWS_BUTTON] 测试代理轮换功能...")
    for i in range(3):
    ...
    print(f"[TREND] 使用统计: 成功请求={stats['successful_requests']}, 失败请求={stats['failed_requests']}")
...
def test_quality_assessment():
    ...
    print(f"[OK] 添加了 {len(test_proxies)} 个不同质量的代理")
    
    # 验证代理
    print("[INSPECT] 验证代理质量...")
    quality_pool.validate_all_proxies()
    
    # 获取高质量代理
    high_quality = quality_pool.get_high_quality_proxies()
    print(f"[ANALYTICS] 高质量代理数: {len(high_quality)} (阈值: {quality_pool.min_quality_score})")
...
def test_smart_rotation():
    ...
    print(f"[OK] 创建了 {len(quality_pool.proxies)} 个代理用于轮换测试")
    
    # 执行多轮选择，观察轮换模式
    print("[COUNTERCLOCKWISE_ARROWS_BUTTON] 执行轮换测试...")
    selected_proxies = []
    for i in range(8):
    ...
    print(f"[TREND] 使用统计: 总请求数={stats['total_requests_made']}")
...
def test_monitoring_system():
    ...
    print(f"[TREND] 当前状态: 总IP={current_status['total_proxies']}, 高质IP={current_status['high_quality_proxies']}")
    
    # 获取详细报告
    detailed_report = monitor.get_detailed_report()
    print(f"[LOG] 部分详细报告预览:")
    ...
    print(f"[HINT] 优化建议: {len(recommendations)} 条")
    ...
    try:
        ...
        print(f"[ANALYTICS] 已生成完整报告 (已截断输出)")
    except ImportError:
        print("[WARNING]  matplotlib未安装，跳过图表生成")
...
def test_integration_with_existing_system():
    ...
    print(f"[OK] 添加了 {added} 个代理到增强池")
    
    # 获取状态
    status = enhanced_pool.get_status()
    print(f"[ANALYTICS] 增强池状态: {status['basic_stats']}")
    print(f"[TREND] 集成池状态: 高质IP={status['integrated_pool_stats']['pool_status']['high_quality_proxies']}")
    
    # 测试代理获取
    for domain in ["example.com", "test.com", "api.site.com"]:
        proxy = enhanced_pool.get_proxy(domain)
        if proxy:
            print(f"[COUNTERCLOCKWISE_ARROWS_BUTTON] 为 {domain} 选择代理: {proxy.address}")
        else:
            print(f"[COUNTERCLOCKWISE_ARROWS_BUTTON] 为 {domain} 未找到可用代理")
    
    # 模拟验证
    print("[INSPECT] 模拟代理验证...")
    for _ in range(2):
...
def run_performance_test():
    ...
    print(f"[OK] 快速添加了 20 个代理，耗时: {time.time() - start_time:.3f}秒")
    ...
    print(f"[OK] 10次快速轮换耗时: {rotation_time:.3f}秒, 平均每次: {rotation_time/10*1000:.2f}ms")
    
    # 获取最终状态
    final_status = pool.get_status()
    print(f"[ANALYTICS] 最终状态: 总请求={final_status['overall_stats']['total_requests']}")
