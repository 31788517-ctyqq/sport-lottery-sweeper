...
try:
    ...
    print("[OK] 成功导入 create_api_router 函数")
    
    # 创建路由器
    router = create_api_router()
    print(f"[OK] 路由器创建成功，总共注册了 {len(router.routes)} 个路由")
    ...
    for expected_route in expected_routes:
        ...
        if found:
            ...
            print(f"  [ERROR] {expected_route} (未找到)")
            missing_routes.append(expected_route)
    
    if missing_routes:
        print(f"\n[WARNING]  缺失的关键路由: {missing_routes}")
    else:
        print("\n[SUCCESS] 所有关键路由都已成功注册!")
