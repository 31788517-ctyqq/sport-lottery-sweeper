...
async def test_sporttery_schedule():
    print("开始测试从竞彩官网获取比赛赛程数据...")
    print("="*60)
    
    print("\n1. 测试 sporttery_scraper 获取比赛数据:")
    try:
    ...
    print("\n" + "="*60)
    print("2. 测试 advanced_crawler 获取比赛数据:")
    try:
        matches = await advanced_crawler.crawl_sporttery_matches(3)
        
        if not matches:
            print("advanced_crawler 未能获取到任何比赛数据")
        else:
            print(f"advanced_crawler 成功获取到 {len(matches)} 场比赛数据")
            
            # 显示前5场比赛的基本信息
            print(f"\n📊 advanced_crawler 获取的前5场比赛:")
...
if __name__ == "__main__":
    ...
    asyncio.run(test_sporttery_schedule())