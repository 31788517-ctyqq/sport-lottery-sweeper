...
async def test_create_and_edit_datasource_precise():
    """精确测试：创建和编辑数据源的完整流程"""
    async with async_playwright() as p:
        ...
        try:
            ...
            print("6. 精确选择分类...")
            # 使用 nth(0) 选择第一个匹配的元素
            category_select = page.locator('.el-form-item').filter(has_text="分类").locator('.el-select').first
            ...
            try:
                ...
                await page.screenshot(path='precise_final_test_creation_failed.png')
                return  # 如果创建失败则退出测试
            ...
            try:
                ...
                await page.screenshot(path='precise_final_test_update_failed.png')
                
            print("✅ 精确测试流程结束")
                
        except Exception as e:
            print(f"测试过程中出现错误: {str(e)}")
            await page.screenshot(path='precise_final_test_error.png')
            raise e
        finally:
            await browser.close()
...
if __name__ == "__main__":
    asyncio.run(test_create_and_edit_datasource_precise())