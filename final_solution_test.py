...
async def test_with_force_click():
    """使用force参数强制点击元素"""
    async with async_playwright() as p:
        ...
        try:
            ...
            print("6. 强制点击分类选择器...")
            # 使用force=True参数强制点击，绕过可见性和稳定性的检查
            category_select = page.locator('label:has-text("分类")').locator('xpath=following-sibling::div[1] .el-select__wrapper').first
            ...
            try:
                ...
                await page.screenshot(path='final_solution_test_creation_failed.png')
                return  # 如果创建失败则退出测试
            ...
            try:
                ...
                await page.screenshot(path='final_solution_test_update_failed.png')
                
            print("✅ 最终解决方案测试流程结束")
                
        except Exception as e:
            print(f"测试过程中出现错误: {str(e)}")
            await page.screenshot(path='final_solution_test_error.png')
            raise e
        finally:
            await browser.close()
...
if __name__ == "__main__":
    asyncio.run(test_with_force_click())