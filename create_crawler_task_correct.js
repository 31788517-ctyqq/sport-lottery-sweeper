...
/**
 * 为新数据源创建爬虫任务（使用正确的参数格式）
 */
async function createCrawlerTaskCorrect() {
  try {
    ...
    // 发送POST请求创建爬虫任务，使用params而不是data
    console.log('\n--- 正在创建爬虫任务 ---');
    const response = await axios.post('http://localhost:8000/api/v1/admin/tasks', null, {
      params: taskData
    });
