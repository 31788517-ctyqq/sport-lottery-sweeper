...
export const batchDeletePermissions = (ids) => {
  return http.delete('/admin/permissions/batch', { data: { ids } })
}

// 更新权限状态
export const updatePermissionStatus = (id, status) => {
  return http.patch(`/admin/permissions/${id}/status`, { status })
}

// 获取权限树
export const getPermissionTree = (params) => {
  return http.get('/admin/permissions/tree', { params })
}

// 获取权限分组
export const getPermissionGroups = () => {
  return http.get('/admin/permissions/groups')
}

// 根据类型获取权限
export const getPermissionsByType = (type) => {
  return http.get(`/admin/permissions/type/${type}`)
}

// 检查权限编码是否存在
export const checkPermissionCode = (code, excludeId) => {
  return http.get('/admin/permissions/check-code', { params: { code, excludeId } })
}

// 获取权限统计信息
export const getPermissionStats = () => {
  return http.get('/admin/permissions/stats')
}

// 同步权限（从代码自动生成）
export const syncPermissions = () => {
  return http.post('/admin/permissions/sync')
}
...
export const exportPermissions = (params) => {
  return http.get('/admin/permissions/export', { params, responseType: 'blob' })
}
