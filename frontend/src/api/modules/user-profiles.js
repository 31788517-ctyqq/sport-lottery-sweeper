...
export const getUserProfiles = async (params = {}) => {
  ...
  const page = Number.isFinite(Number(params.page)) ? Number(params.page) : Math.floor(skip / Math.max(limit, 1)) + 1
  const size = Number.isFinite(Number(params.size)) ? Number(params.size) : limit
  ...
  try {
    return await http.get('/api/v1/admin/user-profiles/', { params: pageParams })
  } catch (error) {
    if (error?.response?.status === 422) {
      return http.get('/api/v1/admin/user-profiles/', { params: skipParams })
    }
    throw error
  }
}
