<template>
    ...
    <el-card class="box-card">
      ...
      <el-table :data="tableData" border style="width: 100%" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="源类型" width="120">
          <template #default="scope">
            <el-tag type="info">{{ getSourceTypeLabel(scope.row) }}</el-tag>
          </template>
        </el-table-column>
  ...
  <el-dialog v-model="addDialogVisible" title="新增数据源" width="500px">
    <el-form :model="formData" label-width="80px" class="dialog-form">
      ...
      <el-form-item label="源类型">
        <el-select v-model="formData.source_type" style="width: 100%;" @change="handleAddSourceTypeChange">
          <el-option
  ...
  <el-dialog v-model="add100qiuDialogVisible" title="创建100球数据源" width="500px">
    <el-form :model="form100qiuData" label-width="120px" class="dialog-form">
      <el-form-item label="date_time" required>
        <el-input v-model="form100qiuData.date_time" placeholder="请输入date_time参数（latest 或 260155）" />
      </el-form-item>
  ...
  <el-dialog v-model="editDialogVisible" title="编辑数据源" width="500px">
    <el-form :model="editFormData" label-width="80px" class="dialog-form">
      ...
      <el-form-item v-if="!editFormData.is100qiu" label="源类型">
        <el-select v-model="editFormData.source_type" style="width: 100%;" @change="handleEditSourceTypeChange">
          <el-option
            v-for="item in sourceTypeOptions"
            :key="item.value"
            :label="item.label"
      ...
      <el-form-item v-if="editFormData.is100qiu" label="date_time" required>
        <el-input v-model="editFormData.date_time" placeholder="请输入date_time参数（latest 或 260155）" />
      </el-form-item>
...
const SOURCE_API_BASE = '/api/v1/admin/sources'
const sourceTypeOptions = [
  { label: '100球', value: '100qiu' },
  { label: '500w', value: '500w' },
  { label: '盈球', value: 'yingqiu' },
  { label: 'Kaggle', value: 'kaggle' },
  { label: '其他', value: 'other' }
]
const sourceTypeLabelMap = sourceTypeOptions.reduce((acc, item) => {
  acc[item.value] = item.label
  return acc
}, {})
const sourceTypeUrlTemplateMap = {
  '100qiu': 'https://m.100qiu.com/api/dcListBasic',
  '500w': 'https://trade.500.com/bjdc/',
  'yingqiu': 'https://www.ttyingqiu.com/bjdc',
...
const normalizeSourceType = (value) => {
  ...
  if (!raw) return 'other'
  if (raw.includes('100qiu') || raw.includes('100')) return '100qiu'
  if (raw.includes('500w') || raw.includes('500')) return '500w'
  if (raw.includes('yingqiu')) return 'yingqiu'
  if (raw.includes('kaggle')) return 'kaggle'
  return 'other'
}

const resolveSourceType = (row) => {
  const config = parseConfigObject(row?.config)
  const fromConfig = normalizeSourceType(config.source_type)
  if (fromConfig !== 'other') return fromConfig

  const fromName = normalizeSourceType(row?.name)
  if (fromName !== 'other') return fromName

  const fromUrl = normalizeSourceType(row?.url)
  if (fromUrl !== 'other') return fromUrl

  return 'other'
}

const getSourceTypeLabel = (row) => {
  const type = resolveSourceType(row)
  return sourceTypeLabelMap[type] || '其他'
}

const applySourceTypeTemplate = (targetForm, sourceType) => {
  if (!targetForm || !sourceType) return
  const templateUrl = sourceTypeUrlTemplateMap[sourceType]
  if (templateUrl && (!targetForm.url || !String(targetForm.url).trim())) {
    targetForm.url = templateUrl
  }
  if (!targetForm.category && ['100qiu', '500w', 'yingqiu', 'kaggle'].includes(sourceType)) {
    targetForm.category = 'match_data'
  }
}

const handleAddSourceTypeChange = (value) => {
  applySourceTypeTemplate(formData.value, value)
}

const handleEditSourceTypeChange = (value) => {
  applySourceTypeTemplate(editFormData.value, value)
}
...
const loadData = async () => {
  try {
    ...
    const url = `${SOURCE_API_BASE}${queryString}`
...
const getCategory = (row) => {
  // 100球数据源分类固定为比赛数据
  if (resolveSourceType(row) === '100qiu') {
    return '比赛数据'
  }
  const config = parseConfigObject(row?.config)
  const category = config.category || 'match_data'
...
// 判断是否为100球数据源
const is100qiuDataSource = (row) => resolveSourceType(row) === '100qiu'
...
const saveDataSource = async () => {
  try {
    ...
    const config = {
      ...
      source_type: normalizeSourceType(formData.value.source_type)
    }
    
    const response = await fetch(SOURCE_API_BASE, {
      method: 'POST',
      headers: {
...
const save100qiuDataSource = async () => {
  try {
    ...
    // 验证 date_time 格式：支持 latest 或 5-6 位数字
    const date_time = form100qiuData.value.date_time.toString().trim()
    if (date_time !== 'latest' && !/^\d{5,6}$/.test(date_time)) {
      ElMessage.warning('date_time必须是latest或5-6位数字（如：260155）')
      return
    }
...
const handleEdit = (row) => {
  ...
  editFormData.value.source_type = resolveSourceType(row)
  ...
  if (is100qiu) {
    // 100球数据源 - 分类固定为比赛数据
    const config = parseConfigObject(row.config)
    editFormData.value.date_time = config.date_time || 'latest'
...
const deleteDataSource = async (id) => {
  try {
    const response = await fetch(`${SOURCE_API_BASE}/${id}`, {
      method: 'DELETE',
      credentials: 'include'
    })
    const data = await parseJsonResponse(response)
...
const checkHealth = async (row) => {
  try {
    const response = await fetch(`${SOURCE_API_BASE}/${row.id}/health`, {
      method: 'POST',
      credentials: 'include'
    })
    const data = await parseJsonResponse(response)
...
const batchDelete = () => {
  ...
  ElMessageBox.confirm(`确定要删除选中的 ${selectedIds.value.length} 个数据源吗？`, '警告', {
    ...
    try {
      const response = await fetch(`${SOURCE_API_BASE}/batch`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  try {
    ...
    if (editFormData.value.is100qiu) {
      ...
      config = {
        ...
        source_type: normalizeSourceType(editFormData.value.source_type)
      }
    }
    
    const response = await fetch(`${SOURCE_API_BASE}/${editFormData.value.id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json'
      },
