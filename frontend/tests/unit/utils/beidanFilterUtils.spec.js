// Unit tests for BeidanFilter utilities
...
describe('BeidanFilter Utilities', () => {
  describe('calcDeltaPLevel', () => {
    ...
    it('handles string values representing numbers', () => {
      expect(calcDeltaPLevel('50', '40')).toBe(1);
      expect(calcDeltaPLevel('40', '50')).toBe(-1);
    });

    it('returns 0 when values are "-"', () => {
      expect(calcDeltaPLevel('-', '40')).toBe(0);
      expect(calcDeltaPLevel('50', '-')).toBe(0);
      expect(calcDeltaPLevel('-', '-')).toBe(0);
    });
  });

  describe('calcDeltaWp', () => {
    it('correctly calculates win pan difference', () => {
      // 2.0 -> score 4, 0.5 -> score 0, difference = 4 - 0 = 4
      expect(calcDeltaWp(2.0, 0.5)).toBe(4);
      // 1.3 -> score 3, 0.5 -> score 0, difference = 3 - 0 = 3
      expect(calcDeltaWp(1.3, 0.5)).toBe(3);
      // 1.0 -> score 2, 0.5 -> score 0, difference = 2 - 0 = 2
      expect(calcDeltaWp(1.0, 0.5)).toBe(2);
      // 0.7 -> score 1, 0.5 -> score 0, difference = 1 - 0 = 1
      expect(calcDeltaWp(0.7, 0.5)).toBe(1);
      // 0.5 -> score 0, 0.5 -> score 0, difference = 0 - 0 = 0
      expect(calcDeltaWp(0.5, 0.5)).toBe(0);
    });
  });

  describe('calcStabilityTier', () => {
    it('correctly calculates stability tier', () => {
      // Test case where both teams have high one-odds percentage
      const result1 = calcStabilityTier('一赔70%', '一赔75%');
      expect(result1.tier).toBe('S');
      expect(result1.pLevel).toBe(1);

      // Test case where both teams have medium one-odds percentage
      const result2 = calcStabilityTier('一赔60%', '一赔55%');
      expect(result2.tier).toBe('A');
      expect(result2.pLevel).toBe(2);

      // Test case where one team has one-odds feature
  ...
  describe('normalizeMatches', () => {
    it('correctly normalizes match data', () => {
      const mockMatches = [{
        match_id: '123',
        home_team: '主队',
        away_team: '客队',
    ...
    it('handles missing or invalid data', () => {
      const mockMatches = [{
        // Some fields missing
        match_id: '124',
        power_home: '-', // Invalid data
        power_away: '-'  // Invalid data
      }];

      const normalized = normalizeMatches(mockMatches);
      expect(normalized).toHaveLength(1);
      expect(normalized[0].power_diff).toBe(0); // Should handle invalid data gracefully
      expect(normalized[0].win_pan_diff).toBe(0); // Default value
    });
  });
});