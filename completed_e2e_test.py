...
async def completed_e2e_test():
    """最终完成的端到端测试，验证数据源管理页面的所有功能"""
        ...
        try:
            ...
            if table_cards > 0:
                ...
                if rows > 0:
                    ...
                    # 检查内容分类列
                    category_cells = await page.locator('td:has-text("比赛数据"), td:has-text("赔率数据"), td:has-text("赛事数据")').count()
                    print(f"🏷️ 内容分类单元格: {category_cells}")
            ...
            # 选择分类 - 使用更精确的选择器，选择表单中的分类字段（而不是筛选区的）
            ...
            # 等待弹窗关闭（检查新增弹窗是否消失）
            await page.wait_for_selector('text="新增数据源"', state='detached', timeout=10000)
            print("✅ 新增弹窗已关闭")
            ...
            if await first_edit_btn.count() > 0:
                ...
                # 等待编辑弹窗关闭
                await page.wait_for_selector('text="编辑数据源"', state='detached', timeout=10000)
                print("✅ 编辑弹窗已关闭")
            ...
            # 11. 验证数据源列表中的分类信息是否正确保存
            print("\n1️⃣1️⃣ 验证分类信息保存...")
            # 检查表格中是否有分类标签
            category_tags = await page.locator('.el-tag').count()
            print(f"🏷️ 分类标签数量: {category_tags}")
            ...
            print("✅ 所有主要功能验证完成")
            print("✅ 分类信息显示和保存功能正常")
            print("✅ 数据源创建、编辑、筛选功能正常")
            print("✅ 批量操作功能正常")
            
        except Exception as e:
            print(f"❌ 测试过程中出现错误: {str(e)}")
            await page.screenshot(path='completed_e2e_test_error.png')
            raise e
        finally:
            await browser.close()
...
if __name__ == "__main__":
    asyncio.run(completed_e2e_test())