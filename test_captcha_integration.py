...
def test_manual_captcha_integration():
    ...
    print("✅ 爬虫已创建并集成人工打码服务")
    print(f"📊 验证码重试次数: {scraper.captcha_integration.captcha_retry_attempts}")
...
def test_captcha_detection():
    ...
    print("✅ 验证码集成服务已创建")
    print(f"📊 重试次数: {captcha_integration.captcha_retry_attempts}")
    print(f"⏰ 检测超时: {captcha_integration.captcha_detection_timeout}秒")
    print(f"💡 重试延迟: {captcha_integration.captcha_retry_delay}秒")
...
def test_different_captcha_services():
    ...
    for service_type in services:
        ...
        try:
            ...
            print(f"   ✅ {service_type} 服务创建成功")
        except Exception as e:
            print(f"   ❌ {service_type} 服务创建失败: {e}")
