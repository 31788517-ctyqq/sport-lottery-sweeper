...
async def test_create_and_edit_datasource_element_ui():
    """使用Playwright内置功能处理Element UI组件的测试"""
    async with async_playwright() as p:
        ...
        try:
            ...
            print("6. 使用Playwright的内置功能选择分类...")
            # 使用get_by_role定位选择器
            category_select = page.locator('label:has-text("分类")').locator('..').locator('.el-select')
            ...
            try:
                ...
                await page.screenshot(path='element_ui_test_creation_failed.png')
                return  # 如果创建失败则退出测试
            ...
            try:
                ...
                await page.screenshot(path='element_ui_test_update_failed.png')
                
            print("✅ Element UI测试流程结束")
                
        except Exception as e:
            print(f"测试过程中出现错误: {str(e)}")
            await page.screenshot(path='element_ui_test_error.png')
            raise e
        finally:
            await browser.close()
...
if __name__ == "__main__":
    asyncio.run(test_create_and_edit_datasource_element_ui())