...
async def test_with_exact_ids():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            print("6. 点击内容分类选择器...")
            # 根据检查结果，使用确切的ID选择器
            # 第二个分类标签的ID是 "el-id-2669-64"，对应的select wrapper是 "el-id-2669-89"
            # 实际上我们需要点击的是select wrapper
            category_select_wrapper = page.locator('#el-id-2669-89').locator('..').locator('.el-select__wrapper')
            await category_select_wrapper.wait_for(timeout=5000)
            await category_select_wrapper.click(force=True)
            ...
            try:
                ...
                await page.screenshot(path='exact_id_test_creation_failed.png')
                return  # 如果创建失败则退出测试
            ...
            try:
                ...
                await page.screenshot(path='exact_id_test_update_failed.png')
                
            print("✅ 确切ID测试流程结束")
                
        except Exception as e:
            print(f"测试过程中出现错误: {str(e)}")
            await page.screenshot(path='exact_id_test_error.png')
            raise e
        finally:
            await browser.close()
...
if __name__ == "__main__":
    asyncio.run(test_with_exact_ids())