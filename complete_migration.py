...
def complete_migration():
    ...
    for league in leagues_data:
        try:
            full_league = league + (now, now, 0)  # Add created_at, updated_at, is_deleted
            cursor.execute("""
            INSERT INTO leagues (
            ...
            # Get the inserted ID
            cursor.execute("SELECT id FROM leagues WHERE code = ?", (league[1],))
            result = cursor.fetchone()
            if result:
                league_code_to_id[league[1]] = result[0]
    ...
    conn.commit()
    print(f"Leagues processed: {len(league_code_to_id)}")
    ...
    for team in teams_data:
        ...
        if league_code in league_code_to_id:
            league_id = league_code_to_id[league_code]
            try:
                cursor.execute("""
                INSERT INTO teams (name, short_name, code, country, league_id, is_active, created_at, updated_at, is_deleted)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (team[0], team[1], team[2], team[3], league_id, team[5], now, now, 0))
                teams_inserted += 1
            except sqlite3.IntegrityError as e:
            ...
            print(f"League {league_code} not found for team {team[0]}")
