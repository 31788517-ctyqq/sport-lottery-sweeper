...
def migrate_correct_fk():
    ...
    conn.commit()
    print(f"Leagues inserted: {len(league_code_to_id)}")
    
    # 现在插入球队，使用正确的league_id
    teams_data = [
    ...
    for team in teams_data:
        ...
        if league_code in league_code_to_id:
            ...
            try:
                ...
                VALUES (?, ?, ?, ?, ?, ?)
                """, (team[0], team[1], team[2], team[3], league_id, team[5]))
            except Exception as e:
    ...
    conn.close()
    print("Migration with correct foreign keys completed!")
