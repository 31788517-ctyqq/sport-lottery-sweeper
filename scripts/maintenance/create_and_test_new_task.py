...
def create_and_test_task_with_correct_source():
    db = next(get_db())
    try:
        # 找到100qiu相关的爬虫配置
        ...
        if not qiu_config:
            ...
            ds = db.query(DataSource).filter(DataSource.name.contains('100球')).first()
            if ds:
                qiu_config = CrawlerConfig(
                    name=f"Crawler-{ds.name}",
                    description=f"Configuration for {ds.name}",
                    url=ds.url,
                    frequency=3600,
                    is_active=True,
                    source_id=ds.id,
                ...
                db.add(qiu_config)
                db.commit()
                print(f"创建了新的100qiu爬虫配置，ID: {qiu_config.id}")
            else:
                print("没有找到100qiu数据源")
                return
...
if __name__ == '__main__':
    create_and_test_task_with_correct_source()