"""
数据源配置页面功能测试脚本
"""
import urllib.request
import urllib.error
import json

BASE_URL = "http://localhost:8000"

def test_api_endpoint(url, method="GET", data=None, headers=None):
    """测试API端点"""
    try:
        if headers is None:
            headers = {}
        
        if data:
            req_data = json.dumps(data).encode('utf-8')
            req = urllib.request.Request(url, data=req_data, method=method, 
                                       headers={**headers, 'Content-Type': 'application/json'})
        else:
            req = urllib.request.Request(url, method=method, headers=headers)
        
        with urllib.request.urlopen(req, timeout=10) as response:
            result = response.read().decode('utf-8')
            return response.getcode(), result
    except urllib.error.HTTPError as e:
        # HTTP错误也是有意义的信息，返回状态码和错误内容
        try:
            error_body = e.read().decode('utf-8')
            return e.code, error_body
        except:
            return e.code, str(e)
    except urllib.error.URLError as e:
        return 0, f"URL Error: {e.reason}"
    except Exception as e:
        return 0, f"Error: {str(e)}"

def main():
    print("测试数据源配置页面相关API功能...")
    
    # 检查后端服务是否运行
    try:
        with urllib.request.urlopen(f"{BASE_URL}/docs", timeout=5) as response:
            if response.getcode() in [200, 404, 405]:
                print("✓ 后端服务连接正常")
            else:
                print(f"✗ 后端服务连接异常: {response.getcode()}")
                return
    except Exception as e:
        print(f"✗ 后端服务未运行或无法连接: {e}")
        return
    
    print("\n" + "="*80)
    print("数据源管理API功能验证")
    print("="*80)
    
    # 测试数据源管理API
    test_cases = [
        # 数据源列表API
        ("数据源列表", f"{BASE_URL}/api/v1/admin/sources?page=1&size=10", "GET"),
        
        # 尝试获取一个具体的数据源（如果没有则测试错误处理）
        ("获取数据源详情", f"{BASE_URL}/api/v1/admin/sources/1", "GET"),
        
        # 测试连接API
        ("测试连接", f"{BASE_URL}/api/v1/admin/sources/1/test-connection", "POST"),
    ]
    
    results = []
    for name, url, method in test_cases:
        status, result = test_api_endpoint(url, method)
        results.append((name, status, result))
        print(f"{name}: {status}")
        
        # 根据状态码给出解释
        if status == 200:
            print(f"  ✓ {name} API正常工作")
        elif status == 401:
            print(f"  ⚠ {name} API路径正确，需要认证")
        elif status == 404:
            print(f"  ⚠ {name} API路径正确，资源不存在（正常情况）")
        elif status == 405:
            print(f"  ⚠ {name} API路径正确，方法不允许")
        elif status >= 400:
            print(f"  ✗ {name} API可能存在问题: {status}")
        else:
            print(f"  ? {name} API状态未知: {status}")
        print()
    
    print("-" * 80)
    print("数据源API验证摘要")
    print("-" * 80)
    
    total_tests = len(results)
    successful_responses = sum(1 for _, status, _ in results if status in [200, 401, 404, 405])
    error_responses = sum(1 for _, status, _ in results if status >= 400 and status not in [401, 404, 405])
    
    print(f"总测试数: {total_tests}")
    print(f"成功/预期错误数: {successful_responses}")
    print(f"错误数: {error_responses}")
    
    if error_responses == 0:
        print("\n✅ 数据源管理API功能正常！")
    else:
        print(f"\n⚠️  {error_responses} 个API端点存在问题")
    
    # 检查前端页面是否存在
    print("\n" + "="*80)
    print("前端页面可用性验证")
    print("="*80)
    
    frontend_url = "http://localhost:3000/admin/data-source/config"
    try:
        with urllib.request.urlopen(frontend_url, timeout=5) as response:
            print(f"前端页面: {response.getcode()}")
            if response.getcode() == 200:
                print("✅ 前端页面访问正常")
            else:
                print(f"⚠️ 前端页面返回状态: {response.getcode()}")
    except urllib.error.HTTPError as e:
        print(f"前端页面: {e.code}")
        print(f"⚠️ 前端页面返回HTTP错误: {e.code}")
        # 对于Vue Router应用，SPA通常会在根路径返回HTML，其他路径也可能返回同样的HTML
        # 所以前端路由404并不一定代表页面不存在
    except urllib.error.URLError as e:
        print(f"❌ 前端页面访问失败: {e.reason}")
        print("💡 提示: 请确认前端服务是否在3000端口运行")

if __name__ == "__main__":
    main()