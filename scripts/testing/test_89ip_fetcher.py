...
def test_89ip_integration():
    ...
    try:
        ...
        print(f"✅ 从89ip.cn获取完成，耗时: {elapsed:.2f}秒")
        print(f"📈 成功添加: {added_count} 个IP")
        
        # 获取更新后的状态
        updated_status = pool.get_proxy_count()
        print(f"📊 更新后IP池状态: 总计={updated_status['total']}, 活跃={updated_status['active']}, 失败={updated_status['failed']}")
        ...
        if added_count > 0:
            ...
            for proxy in pool.proxies[-min(3, added_count):]:  # 验证最后3个
                ...
                if pool.validate_proxy(proxy):
                    ...
                    verified_count += 1
                    print(f"   ✅ {proxy.address} 验证成功")
                else:
                    pool.mark_proxy_failed(proxy)
                    print(f"   ❌ {proxy.address} 验证失败")
            
            print(f"📈 验证结果: {verified_count}/3 个IP可用")
            
            # 更新最终状态
            final_status = pool.get_proxy_count()
            print(f"📊 最终IP池状态: 总计={final_status['total']}, 活跃={final_status['active']}, 失败={final_status['failed']}")
    
    except Exception as e:
        print(f"❌ 获取IP时出错: {str(e)}")
        import traceback
        traceback.print_exc()
